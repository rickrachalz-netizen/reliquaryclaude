...
- `Content/Resources/BP_ResourcePickup` → **`ARLResourcePickup`** (nodes left on the native PickupClass default automatically use this Blueprint when it exists — the native pickup has no mesh, so give this one a visible mesh)
  - Parent `BP_Tree`/`BP_Stone` (and other node variants) to **`BP_ResourceNode`**, not directly to the C++ class — the shared OnShattered presentation (stump/debris/falling-prop spawns) lives in BP_ResourceNode's graph and only reaches children of it.
